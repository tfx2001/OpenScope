...
Window::Window() : m_console(Console::OPENOCD_WINDOW_NAME) {
	...
	m_sidebar.setConnectCallback([&](const std::string& intf, const std::string& target) {
		return m_openocd.startProcess(intf, target,
			[&](std::string&& msg) { m_console.appendLine(std::forward<std::string>(msg)); });
		});
}
...
void Window::resetLayout() {
	...
	openocdOutputId = ImGui::DockBuilderSplitNode(
		mainWindowId, ImGuiDir_Down, 0.3f, nullptr, &mainWindowId);
