...
void Window::initGLFW() {
    ...
    this->m_window = glfwCreateWindow(800, 480, this->m_window_title.c_str(), nullptr, nullptr);
...
void Window::resetLayout() {
    ...
    sidebarId = ImGui::DockBuilderSplitNode(
            dockSpaceId, ImGuiDir_Left, 0.3f, nullptr, &mainWindowId);
    ImGui::DockBuilderDockWindow(Sidebar::WINDOW_NAME, sidebarId);
    ImGui::DockBuilderDockWindow("MainWindow", mainWindowId);
    ImGui::DockBuilderFinish(dockSpaceId);
}

void Window::initUI() {
    this->m_sidebar = std::make_unique<Sidebar>();
    this->m_popup = std::make_unique<Popup>();
    (void) this->m_sidebar;
    (void) this->m_popup;
}
